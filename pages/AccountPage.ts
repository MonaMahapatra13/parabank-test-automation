...
export class AccountPage {
  ...
  async openNewSavingsAccount(): Promise<string> {
    ...
    const options = await this.page.$$eval('#fromAccountId option', (opts: HTMLOptionElement[]) =>
    ...
    await Promise.all([
      this.page.waitForResponse(
        response =>
          response.url().includes('services_proxy/bank/createAccount') && response.status() === 200,
      ),
    ...
    const accountNumberText = await this.page.locator('#newAccountId').innerText();
  ...
  async verifyAccountInOverview(accountNumber: string): Promise<void> {
    ...
    // Validate balance format (allowing for negative values)
